...
public class UserActivity {
    ...
    private void processRequest(HttpResponse<String> response) {
        ...
        for (int i = 0; i < eventsJson.size(); i++) {
            ...
            String repoUrl = repo.get("url").getAsString();

            JsonObject payload = event.get("payload").getAsJsonObject();

            String action = "null";
            if (payload.has("action")) action = payload.get("action").getAsString();

            Event e = new Event(id, eventType, repoName, repoUrl, createdAtDate, action);
